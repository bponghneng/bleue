...
class TestWorkerOptions:
    ...
    def test_worker_options_includes_all_workers(self):
        ...
        assert "executor-1" in worker_ids, (
            f"expected executor-1 to be in worker_ids, got {worker_ids}"
        )
        assert "executor-2" in worker_ids, (
            f"expected executor-2 to be in worker_ids, got {worker_ids}"
        )
        assert "executor-3" in worker_ids, (
            f"expected executor-3 to be in worker_ids, got {worker_ids}"
        )
...
class TestGetWorkerDisplayName:
    ...
    def test_executor_workers(self):
        ...
        assert (
            get_worker_display_name("executor-1") == "Executor 1"
        ), "expected 'executor-1' to display as 'Executor 1'"
        assert (
            get_worker_display_name("executor-2") == "Executor 2"
        ), "expected 'executor-2' to display as 'Executor 2'"
        assert (
            get_worker_display_name("executor-3") == "Executor 3"
        ), "expected 'executor-3' to display as 'Executor 3'"
